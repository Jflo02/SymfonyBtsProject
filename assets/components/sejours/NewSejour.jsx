...
export default function NewSejour({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        serverAddress + "/api/sejours",
        nouveauSejourRef.current
      );
  ...
  const formatDate = (date) => {
    setDateHook(date);
    let yourdate = new Intl.DateTimeFormat(["ban", "id"])
      .format(date)
      .split("/")
      .reverse()
      .join("-");
  ...
  const handleSelectService = (selectedOption) => {
    serviceSejourRef.current.service =
      "/api/services/" + selectedOption.target.value;
  };
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="row">
          ...
          <select
            className="m-3 form-selec form-select-sm w-25"
            name="lits"
            onChange={(lit) =>
              (nouveauSejourRef.current.lit =
                "/api/lits/" + Number(lit.target.value))
            }
          >
